...
enum ServerService {
    ...
    private struct AddOrderData: Codable {
        let products: [Int64]
        let quantity: [Int]
    ...
    static func addOrder(products: [Product:Int], cardNumber: String, expirationDate: String, cvv: String) async throws -> AddOrderResponse {
        var request = try prepareRequest("http://127.0.0.1:8000/add_order")
        
        let sortedPairs = products.sorted { $0.key.name! < $1.key.name! }
        let ids = sortedPairs.map { $0.key.id }
        let quantities = sortedPairs.map { $0.value }
        
        let body = AddOrderData(products: ids, quantity: quantities, card_number: cardNumber, expiry_date: expirationDate, cvv: cvv)
        request = try parseRequestData(request, body)
        let data = try await callServer(request)
        
        do {
            let response = try JSONDecoder().decode(AddOrderResponse.self, from: data)
            print(response)
            return response
        } catch {
    ...
    static func addProduct(categoryId: Int64, name: String, price: Double, description: String) async throws -> Int64 {
        ...
        let body = AddProductData(category_id: categoryId, name: name, price: price, description: description)
        request = try parseRequestData(request, body)
        let data = try await callServer(request)
